...
def reduce_pseudos(
    ...
    origin_pseudos = deepcopy(pseudos)
    reduced_pseudos = {}
    if structure is not None:
        for kind in structure.kinds:
            if kind.name not in origin_pseudos:
                raise ValueError(
                    f"{kind.name} do not match the pesudos.\n", origin_pseudos
                )
            if kind.symbol not in reduced_pseudos:
                reduced_pseudos[kind.symbol] = origin_pseudos[kind.name]
            else:
                if reduced_pseudos[kind.symbol] != origin_pseudos[kind.name]:
                    raise NotImplementedError(
                        "Same element different pseudos is not implement."
                    )
    else:
        for _, val in origin_pseudos.items():
